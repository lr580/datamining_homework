...
> **知识摘要：**层次聚类是每次选择符合给定标准的最小两个类，将其聚合为一类，不断重复这个过程直到聚类完成，并保留聚类步骤。
>
> 1. 最小聚类：组间最小边最小的两类聚合
> 2. 最大聚类：组间最大边最小的两类聚合
...
2. 初始化策略：我们探索了多种不同的初始化策略 

   > 下图中展示的随机种子=580，但测试过程试过其他多个随机数种子

探索的初始化具体如下：

1. 朴素策略为均匀 $w_i$，随机选点做 $\mathbf\mu_i$，且 $\mathbf\Sigma=k\mathbf I_n$，效果不佳。所以考虑更换初始化策略。

   <img src="img/gmm_init_random.png" alt="gmm_init_random" style="zoom:50%;" />

2. 使用Kmeans，效果好一点。这里展示随机数种子seed=50的结果：

   

   <img src="img/gmm_init_kmeans_.png" alt="gmm_init_kmeans_" style="zoom:50%;" />

3. 进一步使用Kmeans++改进，效果更佳。

4. 我们还进行了Kmedoids 尝试，但效果不佳。
